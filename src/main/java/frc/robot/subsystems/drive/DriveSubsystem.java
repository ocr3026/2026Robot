...
public class DriveSubsystem extends SubsystemBase implements Vision.VisionConsumer {
  ...
  private final Module[] modules =
      new Module[4]; // Front Left (FL), Front Right (fr), Back Left (BL), Back Right (BR)

  private final SysIdRoutine sysId;
  private final Alert gyroDisconnectedAlert =
      new Alert("Gyro disconnected, using kinematics instead", AlertType.kError);
  ...
  public double getDeltaRotation(Pose2d pose) {
    ...
    Logger.recordOutput("TypeScript/robotPoseTheta", robotToPoseTheta);
